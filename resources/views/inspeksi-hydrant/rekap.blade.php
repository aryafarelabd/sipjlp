...
@section('title', 'Rekap Pemeriksaan Hydrant Outdoor')
    ...
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            <div class="col">
                ...
                <h2 class="page-title">Rekap Pemeriksaan Hydrant Outdoor</h2>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="container-xl">
        ...
        <div class="card mb-3">
            <div class="card-body py-2">
                <form method="GET" action="{{ route('inspeksi-hydrant.rekap') }}" class="row g-2 align-items-end">
                    <div class="col-sm-2">
            ...
            <div class="card-footer d-flex justify-content-end py-2 border-top">
                @include('exports.partials.buttons', ['route' => 'export.inspeksi-hydrant', 'params' => ['bulan' => $bulan, 'tahun' => $tahun, 'search' => $search]])
            </div>
            </div>
        </div>
        ...
        <div class="row row-deck row-cards mb-3">
            ...
            <div class="col-sm-4">
                <div class="card card-sm">
                    <div class="card-body text-center">
                        <div class="text-muted small">Rata-rata Lokasi Aman</div>
                        @php $rataAman = $logbooks->count() > 0 ? round($logbooks->avg('lokasi_aman'), 1) : 0; @endphp
                        <div class="h2 mb-0 {{ $rataAman >= 4 ? 'text-success' : ($rataAman >= 3 ? 'text-warning' : 'text-danger') }}">
                            {{ $rataAman }}/5
                        </div>
                    </div>
                </div>
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title"><i class="ti ti-table me-1"></i>Data Pemeriksaan Hydrant</h3>
            ...
            <div class="card-body">
                <div class="empty py-4">
                    ...
                    <p class="empty-subtitle text-muted">Belum ada laporan pemeriksaan hydrant untuk periode ini.</p>
                </div>
            </div>
            @else
                ...
                <table class="table table-vcenter card-table table-sm">
                    <thead>
                        <tr>
                            ...
                            <th>Petugas</th>
                            <th>Shift</th>
                            @foreach($lokasi as $lokLabel)
                            <th class="text-center" style="font-size:11px;">{{ Str::limit($lokLabel, 20) }}</th>
                            @endforeach
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            @foreach(array_keys($lokasi) as $lokKey)
                            @php
                                $data   = $log->$lokKey ?? [];
                                $allOk  = true;
                                foreach(\App\Models\InspeksiHydrant::NILAI_AMAN as $k => $v) {
                                    if (($data[$k] ?? null) !== $v) { $allOk = false; break; }
                                }
                            @endphp
